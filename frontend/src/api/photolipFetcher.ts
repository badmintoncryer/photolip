...
import { PhotolipContext } from './photolipContext'

const baseUrl = 'https://jhdfex74j7.execute-api.ap-northeast-1.amazonaws.com/{basePath}'

export type ErrorWrapper<TError> = TError | { status: 'unknown'; payload: string }

export type PhotolipFetcherOptions<TBody, THeaders, TQueryParams, TPathParams> = {
  url: string
  method: string
  body?: TBody
  headers?: THeaders
...
export async function photolipFetch<
  TData,
  TError,
...
>({
  ...
  queryParams,
  signal
}: PhotolipFetcherOptions<TBody, THeaders, TQueryParams, TPathParams>): Promise<TData> {
  try {
